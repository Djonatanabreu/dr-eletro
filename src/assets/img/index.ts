export const home = require('./drawer/home.png');
export const calendar = require('./drawer/calendar.png');
export const ideia = require('./drawer/ideia.png');
export const compras = require('./drawer/compras.png');
export const carrinho = require('./drawer/carrinho-de-compras.png');
export const profile = require('./drawer/profile.png');
export const noticias = require('./drawer/noticias.png');
export const sobre = require('./drawer/about.png');
export const about = require('./drawer/about.png');
export const sair = require('./drawer/sair.png');

export const bannerAuth = require('./bannerAuth.png');
export const logoauth = require('./logoauth.png');
export const password = require('./password.png');
export const email = require('./email.png');
export const photo = require('./photo.png');

export const lamp = require('./lamp.png');
export const placeholder = require('./placeholder.png');
export const header = require('./header.png');
// export const logo = require('./logo.png');
