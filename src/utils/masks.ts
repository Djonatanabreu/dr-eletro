export const masks: any = {
  ...
  formatTime: (time: string) => {
    const sanitizedTime = String(time).replace(/\D/g, '');
    const hour = sanitizedTime.substring(0, 2);
    const minutes = sanitizedTime.substring(2, 4);
    const seconds = sanitizedTime.substring(4, 6);
  ...
  currency: (value: string) => {
    ...
    if (string.replace(/\D/g, '') !== '')
      return (parseInt(string.replace(/\D/g, '')) / 100).toLocaleString(
        'pt-BR',
        {
